...
/*   Updated: 2024/06/04 17:22:15 by amaucher         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ClapTrap::attack(const std::string& target)
{
	...
	std::cout << _name << " attacks " << target << " resulting in " << _attack << " damage points" << std::endl;
	_energy -= 9;
}
...
void ClapTrap::takeDamage(unsigned int amount)
{
	if (_hit <= amount)
	{
		std::cout << "ClapTrap " << _name << " did not survive the attack!" << std::endl;
		return;
	}
